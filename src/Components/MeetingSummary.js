...
class MeetingSummary extends React.Component {
  constructor(props) {
    ...
    this.getTabSpecificMeetingsList = this.getTabSpecificMeetingsList.bind(this);
          ...
          <div id="meetingSummary" data-cy="meetingSummary" className="meetingSummary">
            {
              meetings.map((meetingItem, index) => {
                const {startTime, endTime, block, floor, meetingRoom, _id} = meetingItem;
                return (
                  <ul className="card d-inbl" data-cy="ul" key={_id}>
                    <li className="fb" data-cy="summaryHeading">{block + ' - ' + floor + ' - ' + meetingRoom}</li>
                    <li className="padT10" data-cy="startSummary" style={{ textAlign: 'left', paddingLeft: '15px'}}>
                      <span className="fb">Starts at:</span>
                      <div>{formatDate(new Date(startTime))}</div>
                    </li>
                    <li className="padT10" data-cy="endSummary" style={{ textAlign: 'left', paddingLeft: '15px' }}>
